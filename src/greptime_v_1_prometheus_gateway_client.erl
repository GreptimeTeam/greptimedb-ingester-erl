...
-spec handle(prom_pb:promql_request()) ->
    {ok, prom_pb:promql_response(), grpcbox:metadata()} | grpcbox_stream:grpc_error_response() | {error, any()}.
handle(Input) ->
    handle(ctx:new(), Input, #{}).

-spec handle(ctx:t() | prom_pb:promql_request(), prom_pb:promql_request() | grpcbox_client:options()) ->
    {ok, prom_pb:promql_response(), grpcbox:metadata()} | grpcbox_stream:grpc_error_response() | {error, any()}.
