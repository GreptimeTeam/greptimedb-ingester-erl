...
-spec handle(database_pb:greptime_request()) ->
                {ok, database_pb:greptime_response(), grpcbox:metadata()} |
                grpcbox_stream:grpc_error_response() |
                {error, any()}.
handle(Input) ->
    handle(ctx:new(), Input, #{}).

-spec handle(ctx:t() | database_pb:greptime_request(),
             database_pb:greptime_request() | grpcbox_client:options()) ->
                {ok, database_pb:greptime_response(), grpcbox:metadata()} |
                grpcbox_stream:grpc_error_response() |
                {error, any()}.
